...
const models = {}
const years = {}
const makes = Object.keys(lookUps).map(m => {
    const lower = m.toLowerCase()
    models[lower] = []
    Object.keys(lookUps[m]).forEach(mm => {
        const mmLower = mm.toLowerCase()
        years[mmLower] = lookUps[m][mm].map(y => y.toString())
        models[lower].push(mmLower)
    })
    return lower
})
    ...
    return db.collection(`claims`).find({ settlement_id: `S0108` }, { clientClaims: true }).toArray()
        ...
        .then(results => {
            ...
            results.forEach(c => {
                ...
                if (!c.user) {
                    c.invalidReason = `no user record`
                    return invalidClaims.push(c)
                }

                if (!c.user.agreement) {
                    c.invalidReason = `no user agreement`
                    return invalidClaims.push(c)
                }

                if (!c.user.vehicles || !c.user.vehicles.length) {
                    c.invalidReason = `no user vehicles`
                    return invalidClaims.push(c)
                }
                ...
                c.user.vehicleVins.forEach(vehicle => {
                    if (!vehicle.make || !vehicle.model) {
                        c.vehicle = vehicle
                        return invalidReason = `no make or model`
                    }

                    const vmake = vehicle.make.toLowerCase()
                    const makeIndex = makes.indexOf(vmake)
                    const make = makeIndex === -1 ? false : makes[makeIndex]

                    if (!make) {
                        c.vehicle = vehicle
                        return invalidReason = `invalid make (${vehicle.make})`
                    }

                    const vmodel = vehicle.model.toLowerCase()
                    const modelIndex = models[make].indexOf(vmodel)
                    const model = modelIndex === -1 ? false : models[make][modelIndex]

                    if (!model) {
                        c.vehicle = vehicle
                        return invalidReason = `invalid model (${vehicle.model})`
                    }

                    if (years[model].indexOf(vehicle.year.toString()) === -1) {
                        c.vehicle = vehicle
                        return invalidReason = `invalid year`
                    }
            ...
            const Invalids = invalidClaims.map(c => {
                const res = {
                    claimId: c._id,
                    filed: toDate(c.created),
                    userId: c.user ? c.user.userId : ``,
                    userCreated: !c.user ? `` : toDate(c.user.created),
                    VIN: !c.vehicle ? `` : c.vehicle.VIN,
                    year: !c.vehicle ? `` : c.vehicle.year,
                    make: !c.vehicle ? `` : c.vehicle.make,
                    model: !c.vehicle ? `` : c.vehicle.model,
                    class: !c.vehicle ? `` : c.vehicle.class,
                    email: !c.user ? `` : c.user.email,
                    fname: !c.user ? `` : c.user.fname,
                    lname: !c.user ? `` : c.user.lname,
                    name: !c.user ? `` : `${c.user.fname} ${c.user.lname}`,
                    agreement_date: !c.user ? `` : toDate(c.user.agreement),
                    rental: c.rental || 0,
                    towing: c.towing || 0,
                    childCare: c.childCare || 0,
                    repair: c.repair || 0,
            ...
            fs.writeFileSync(path.join(dir, `invalids.json`), JSON.stringify(Invalids))
            ...
            try {
                ...
                fs.writeFileSync(path.join(dir, `claims.csv`), csv)
            } catch (err) {
                console.error(err)
            }
            ...
            const invalidFields = []
            Invalids.forEach(claim => Object.keys(claim).forEach(f => invalidFields.indexOf(f) === -1 ? invalidFields.push(f) : undefined))
            const invalidOpts = { invalidFields, flatten: true }

            try {
                const parser = new Parser(invalidOpts)
                const csv = parser.parse(Invalids)
                fs.writeFileSync(path.join(dir, `invalids.csv`), csv)
            } catch (err) {
                console.error(err)
            }
            ...
            console.log(`invalid: `, Invalids.length)
            console.log(`imagesToGet: `, imagesToGet.length)

            // process.exit()

            return Promise.all(imagesToGet.map(i => {
                if (!i) {
                    return Promise.reject()
                }

                return saveImageToDisk(i[1], path.join(dir, `images`, i[0]))
            }))
                .then(() => {
                    process.exit()
                })
